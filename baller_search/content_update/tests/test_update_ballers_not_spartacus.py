...
class TestGetSubmissionsLinked(BaseCommand):
    ...
    def test_no_description(self):
        text = "\nKhaldor - [reddit](http://www.reddit.com/user/Khaldor)|[twitter](https://twitter.com/#!/Khaldor) \n"
        results = self.command.extract_ballers(text)
        self.assertTrue(len(results) == 1)

    def test_no_twitter(self):
        text = "\nNeedsMoreMinerals - [reddit](http://www.reddit.com/user/NeedsMoreMinerals) - CEO of ONOG  \n"
        results = self.command.extract_ballers(text)
        self.assertTrue(len(results) == 1)

    def test_double_description(self):
        text = "\nAlexHDGamer aka HD - [reddit](http://www.reddit.com/user/AlexHDGamer)|[twitter](https://twitter.com/#!/HDstarcraft) \n"
        results = self.command.extract_ballers(text + text)
        self.assertTrue(len(results) == 2)

    def test_with_description(self):
        text = "\nsantah - [reddit](http://www.reddit.com/user/santah) - creator and manager of [sc2casts.com](http://www.sc2casts.com)  \n"
        results = self.command.extract_ballers(text)
        self.assertTrue(len(results) == 1)
